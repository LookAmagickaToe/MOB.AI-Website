...
const BentoGrid = ({ children, className, ...props }: BentoGridProps) => {
	return (
		<div className={cn("grid w-full lg:auto-rows-[30rem] auto-rows-[24rem] grid-cols-3 gap-2 md:gap-6", className)} {...props}>
			{children}
		</div>
	);
};
...
const BentoCard = ({ name, className, background, Icon, description, footer, ...props }: BentoCardProps) => (
	<div key={name} className={cn("group relative col-span-3 flex flex-col justify-between overflow-hidden rounded-lg md:rounded-xl", "border bg-card hover:border-sky-500", className)} {...props}>
		<div>{background}</div>
		<div className="pointer-events-none z-10 flex transform-gpu flex-col justify-start h-full gap-1 p-6">
